...
fn count_stone_blinking(
    ...
    for &stone in stones {
        let new_stones = transform_stone(stone);
        match total.checked_add(count_stone_blinking(&new_stones, 
            blinks_left - 1, cache)) {
...
fn main() {
    ...
    let result = count_stone_blinking(&input_stones, blinks_left, &mut cache);
